...
class DashboardDataWorker(QThread):
    ...
    Includes try/except blocks to prevent crashing the login view.
    """
    data_loaded = pyqtSignal(dict)

    def __init__(self, api_base_url="http://127.0.0.1:8000"):  # Updated port to 8000 as per your browser screenshots
        super().__init__()
        self.api_base_url = api_base_url

    def run(self):
        # Clean starting dataset structure
        result = {
            "current_stock_pcs": 0,
            "monthly_sales": 0,
            "monthly_expenses": 0,
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=3) as response:
                if response.status == 200:
                    ...
                    # Handle both dictionary wrappers and raw list responses
                    sales_data = raw_res
                ...
                if response.status == 200:
                    ...
                    if isinstance(history_data, list):
                        for item in history_data:
                            # Safely fetch the 'title' sent by your database.py, falling back to description
                            db_title = item.get("title") or item.get("description") or "Generic Transaction"
                            
                            # Avoid duplicate entries comparing database 'title' to UI 'description'
        ...
        # 3. Sort merged list chronologically (Safely inside local scope to protect login)
        ...
        for txn in raw_transactions:
            ...
            # Group for Monthly Calendar summaries
            raw_date = txn["date"]
...
class BlockFlowDashboard(QFrame):
    ...
    def load_live_data(self):
        """Dispatches dynamic worker to read live database logs."""
        self.worker = DashboardDataWorker()
        self.worker.data_loaded.connect(self.on_data_received)
        self.worker.start()

    def on_data_received(self, data):
        """Renders live database statistics directly onto interface."""
        ...
        if not data["recent_transactions"]:
            ...
            for txn in data["recent_transactions"][:5]:  # Display up to 5 latest items
                ...
                is_expense = txn.get("type", "").lower() == "expense"
    ...
    def populate_calendar(self, monthly_history):
        ...
        for index, item in enumerate(monthly_history):
            ...
            month_card.setStyleSheet("""
                QFrame#MonthCard {
                    background-color: rgba(15, 23, 42, 0.5); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px;
                }
            """)
